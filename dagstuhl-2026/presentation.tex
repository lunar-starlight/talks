...
\title{Topological Properties are Taboos in Topological Models}
...
\institute{Dagstuhl Seminar 26121\\
...
\begin{frame}
  ...
  \begin{definicija}[Dedekind construction]\label{def:Rd}
    A pair \(\p{L, U} ∈ 𝒫(ℚ)×𝒫(ℚ)\) is a \emph{Dedekind cut} when
    \begin{enumerate}
...
\begin{frame}
  ...
  \begin{definicija}[MacNeille construction]\label{def:Rm}
    A pair \(\p{L, U} ∈ 𝒫(ℚ)×𝒫(ℚ)\) is a \emph{Dedekind-MacNeille cut} when
    \begin{enumerate}[a.]
...
\begin{frame}
  ...
  \(\alpo*_{\Rm} = \amp*_{\Rm} = \lem*\), \(\awlpo*_{\Rm} = \wlem*\), and
  \(\aks*_{\Rm} = ⊤\).
...
\begin{frame}
  ...
  \begin{trditev}
    \(\c ℝ ≅ \Rc\) holds iff \(\lpo*\) holds.
  \end{trditev}
  \begin{trditev}
    \(\Rd ≅ \Rm\) holds iff \(\wlem*\) holds.
  \end{trditev}
